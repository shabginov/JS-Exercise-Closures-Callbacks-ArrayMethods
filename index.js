...
function firstNamesAllCaps(arr) {
  let allCapsedNames = arr.map(function(runner) {
    return runner.first_name.toUpperCase();
  })
  return allCapsedNames;
}
...
function getRunnersByTShirtSize(arr, size) {
  const specificTshirtArr = arr.filter(function(tShirt) {
    return tShirt.shirt_size === size;
  });
  return specificTshirtArr;
}
...
function tallyUpDonations(arr) {
  const total = arr.reduce((currentTotal, runner) => {
    return runner.donation + currentTotal;
  }, 0);
  return total;
}
